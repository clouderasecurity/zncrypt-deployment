...
which python &>/dev/null || echo "- Installing python." && yum install python -y &>$log_file || warn "Received a bad return code while attempting to install python. Continuing..."
...
test $current_selinux_status = "enforcing" && echo "- Disabling selinux" && setenforce 0
...
for p in ${required_packages[@]}; do
    rpm -qa | grep $p &>$log_file || echo "- Installing package: $p" && yum install $p -y &>$log_file || err "Encountered an error while installing: $p"
done
...
for i in $(seq 1 31); do
    # Sleep for 5 seconds, then check for something (CM) listening on port 7180
    sleep 10
