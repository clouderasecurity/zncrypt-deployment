...
function registerClient {
    test -f /etc/zncrypt/ztrustee/clientname && printf "\n- zNcrypt is already registered. Skipping registration process.\n\n" && return
    ...
    printf "\n==================================================\n"
    printf "\nNote! This is the command we will be using to register zNcrypt:\n"
    printf "\n\$ $register_command\n"
...
function prepareClient {
    ...
    echo '- Where would you like to store the encrypted data? [/var/lib/zncrypt/.private]'
    read storage < /dev/tty
    test -z $storage && storage="/var/lib/zncrypt/.private"
    grep "$storage" /etc/zncrypt/ztab &>/dev/null && err "The location '$storage' is already marked as an encrypted partition. Remove it before continuing."
    ...
    echo '- Where would you like to mount the encrypted partition? [/var/lib/zncrypt/encrypted]'
    read mount < /dev/tty
    test -z $mount && mount="/var/lib/zncrypt/encrypted"
    grep "$mount" /etc/zncrypt/ztab &>/dev/null && err "The location '$mount' is already marked as an encrypted partition. Remove it before continuing."
...
function encryptData {
    ...
    if [[ -z $mount ]]; then
        ...
        if [["${response:0:1}" = "n"] -o ["${response:0:1}" = "N"]]; then
            ...
            printf "- What mount location would you like to use to store this data in? []\n"
            read mount < /dev/tty
            grep "$mount" /etc/zncrypt/ztab &>/dev/null || err "A valid encrypted partition must be specified."
        fi
        unset response
    fi
