...
log_file="/tmp/zncrypt-install.log"
...
function install_zncrypt {
    ...
    case "$operating_system" in
        ...
        rpm -q zncrypt &>$log_file && printf "\t- Already installed.\n" && return 0
        printf "\t- Not present. Installing zNcrypt with yum.\n" && yum install zncrypt -y &>$log_file || err "zNcrypt could not be installed. Stopping."
        chkconfig --level 2345 zncrypt-mount on &>$log_file
        chkconfig --level 2345 haveged on &>$log_file
        ;;
        "ubuntu" | "debian" )
        dpkg --list | grep zncrypt &>$log_file && printf "\t- Already installed.\n" && return 0
        printf "\t- Not present. Installing zNcrypt with apt-get.\n" && apt-get install zncrypt -y &>$log_file
        ;;
        "amazon" )
        rpm -q zncrypt &>$log_file && printf "\t- Already installed.\n" && return 0
        # Begin hack
        printf "\t- Switching to Amazon Linux configuration.\n"
        printf "\t- Removing cryptsetup* packages.\n" && yum remove cryptsetup* -y &>$log_file
        printf "\t- Readding keyutils.\n" && yum install keyutils -y &>$log_file
        printf "\t- Re-installing cryptsetup-luks-libs-1.2.0-7.\n" && wget https://s3.amazonaws.com/gazzang-implementation/cryptsetup-luks-libs-1.2.0-7.el6.x86_64.rpm &>$log_file && rpm -i cryptsetup-luks-libs-1.2.0-7.el6.x86_64.rpm &>$log_file
        printf "\t- Re-installing cryptsetup-luks-1.2.0-7.\n" && wget https://s3.amazonaws.com/gazzang-implementation/cryptsetup-luks-1.2.0-7.el6.x86_64.rpm &>$log_file && rpm -i cryptsetup-luks-1.2.0-7.el6.x86_64.rpm &>$log_file
        printf "\t- Re-installing ecryptfs-utils-82-6.el6_1.3.\n" && wget https://s3.amazonaws.com/gazzang-implementation/ecryptfs-utils-82-6.el6_1.3.x86_64.rpm &>$log_file && rpm -i ecryptfs-utils-82-6.el6_1.3.x86_64.rpm &>$log_file
        printf "\t- Re-installing trousers-0.3.4-4.\n" && wget https://s3.amazonaws.com/gazzang-implementation/trousers-0.3.4-4.el6.x86_64.rpm &>$log_file && rpm -i trousers-0.3.4-4.el6.x86_64.rpm &>$log_file
        rm -f *.rpm &>$log_file
        # End hack
    ...
    which zncrypt &>$log_file || err "zNcrypt could not be installed (install_zncrypt)."
    return 0
}
...
function disable_repositories {
    case "$operating_system" in
        ...
        sed -i s/enabled=1/enabled=0/ /etc/yum.repos.d/gazzang.repo &>$log_file
        if [[ $? -ne 0 ]]; then
...
function remove_zncrypt_ping {
    if [[ -f /etc/cron.hourly/zncrypt-ping ]]; then
        rm -f /etc/cron.hourly/zncrypt-ping &>$log_file
    fi
}

# Check status of kernel module. If not present, build it.
function check_zncryptfs {
    which zncrypt-module-setup &>$log_file || err "zNcrypt not installed (check_zncryptfs). Stopping."
